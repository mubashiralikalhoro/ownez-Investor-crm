...
export function RelatedContacts({ personId, contacts }: RelatedContactsProps) {
  ...
  }, [search, personId, selectedPerson]);

  async function handleAdd() {
    if (!selectedPerson || !role.trim()) return;
    setSaving(true);
    await fetch(`/api/persons/${personId}/related-contacts`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ contactId: selectedPerson.id, role: role.trim() }),
    });
    setSaving(false);
    setAdding(false);
    setSearch("");
  ...
  async function handleRemove(contactId: string) {
    await fetch(`/api/persons/${personId}/related-contacts/${contactId}`, {
      method: "DELETE",
    });
    router.refresh();
  }
