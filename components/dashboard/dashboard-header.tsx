...
import type { PersonWithComputed, User, Activity } from "@/lib/types";
...
function LogActivitySheet({ prospects }: { prospects: PersonWithComputed[] }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/activities?personId=${selectedPerson.id}`)
      .then((r) => r.json())
      .then((data) => setRecentActivities(Array.isArray(data) ? data.slice(0, 5) : []))
  ...
  async function handlePromptConfirm() {
    ...
    setConfirming(true);
    await fetch(`/api/persons/${selectedPerson.id}/next-action`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  async function handleAdvanceStage() {
    ...
    if (currentIdx >= 0 && currentIdx < stages.length - 1) {
      ...
      if (confirm(`Advance ${selectedPerson.fullName} to ${nextStage.replace(/_/g, " ")}?`)) {
        await fetch(`/api/persons/${selectedPerson.id}/stage`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
