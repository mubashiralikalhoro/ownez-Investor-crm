...
export function ProfileCard({ person, users, orgMembers, sessionRole }: ProfileCardProps) {
  ...
  async function saveField(field: string, value: unknown) {
    if (field === "assignedRepId") {
      ...
      const usersList = await demoData.getUsers();
      const newRep = value ? usersList.find((u) => u.id === value) : null;
      const newRepName = newRep?.fullName ?? "Unassigned";
      await demoData.updatePerson(person.id, { assignedRepId: (value as string | null) ?? null });
