...
import { STAGE_LABELS, LOST_REASONS } from "@/lib/constants";
import { getTodayCT } from "@/lib/format";
import { demoData } from "@/data/store";
import type { EntityType, LostReason, Person, PipelineStage, PersonWithComputed } from "@/lib/types";
...
export function StageBar({ currentStage, personId, person, onInlineModeChange }: StageBarProps) {
  ...
  async function applyStageChange(
    stage: PipelineStage,
    extra?: { reengageDate?: string; lostReason?: LostReason }
  ) {
    setSaving(true);
    const p = await demoData.getPerson(personId);
    if (!p) {
      setSaving(false);
      return;
    }
    ...
    if (stage === "nurture" && extra?.reengageDate) updatePayload.reengageDate = extra.reengageDate;
    if (stage === "dead" && extra?.lostReason) updatePayload.lostReason = extra.lostReason;
    await demoData.updatePerson(personId, updatePayload);
    const oldLabel = oldStage ? STAGE_LABELS[oldStage] : "None";
    const newLabel = STAGE_LABELS[stage] || stage;
    ...
    await demoData.createActivity(personId, {
  ...
  async function handleNurtureConfirm() {
    ...
    await applyStageChange("nurture", { reengageDate });
  ...
  async function handleDeadConfirm() {
    ...
    await applyStageChange("dead", { lostReason: lostReason as LostReason });
    setInlineModeWithCallback(null);
    router.refresh();
  }
  ...
  async function handleFundedConfirm() {
    ...
    const today = getTodayCT();
    const entity = await demoData.createFundingEntity({
      entityName,
    ...
    await demoData.createFundedInvestment({
    ...
    const oldStage = p.pipelineStage;
    await demoData.updatePerson(personId, {
      pipelineStage: "funded",
      stageChangedDate: today,
      roles: p.roles.includes("funded_investor")
        ? p.roles
        : [...p.roles.filter((r) => r !== "prospect"), "funded_investor"],
    });
    const oldLabel = oldStage ? STAGE_LABELS[oldStage] : "None";
    const timeStr = new Date().toLocaleTimeString("en-US", {
      hour12: false,
