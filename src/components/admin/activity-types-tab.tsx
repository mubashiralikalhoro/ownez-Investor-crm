...
export function ActivityTypesTab({ types }: ActivityTypesTabProps) {
  ...
  async function saveEdit(key: string) {
    setSaving(true);
    try {
      const updated = await demoData.updateActivityTypeConfig(key, { label: editLabel });
  ...
  async function toggleActive(key: string, isActive: boolean) {
    const updated = await demoData.updateActivityTypeConfig(key, { isActive: !isActive });
    setItems((prev) => prev.map((t) => (t.key === key ? updated : t)));
  }
  ...
  async function addNew() {
    ...
    setAdding(true);
    try {
      const created = await demoData.createActivityType({ label: newLabel.trim() });
